...
def modelInitialization():
    ...
    modelInitialization()
    return 값 model, model_args, training_args, inference_args
    """

    print("model Initialization Strat")
...
def dataPreprocessing(globalPath,timeToCrop=-1):
    """
    ...
    #자르기
    # my_test_sequences = my_test_sequence[0:timeToCrop]
    # my_test_cluster_ids = my_test_cluster_id[0:timeToCrop]
...
def predictWithLabel(model, test_sequences, test_cluster_ids, model_args, training_args, inference_args, outputFileName="./whisperDiarizationOutput.txt"):
    print("predict start")
    start_time = time.time()
    ...
    with open(outputFileName, 'a') as f:
        f.write(f"All predict done :  {time.time() - start_time}s")
